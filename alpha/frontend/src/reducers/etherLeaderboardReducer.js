...
export default function leaderboardReducer(state = new ImmutableMap(initialData), action) {
  ...
  const actions = {
    'ETHER:FETCH_LOAD_LEADERBOARD_REQUEST': () => fetchLoadLeaderboardRequest(state),
    'ETHER:FETCH_LOAD_LEADERBOARD_SUCCESS': () => fetchLoadLeaderboardSuccess(state),
    'ETHER:FETCH_LOAD_LEADERBOARD_FAILURE': () => fetchLoadLeaderboardFailure(state),
    'DEFAULT': () => state
  };
